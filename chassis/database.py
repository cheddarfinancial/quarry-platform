...
# Third Party
import mixingboard
from sqlalchemy import create_engine, types, event, exc, Text
...
user_db_conf = mixingboard.getConf('user_db')

engine = create_engine('mysql+pymysql://%s:%s@%s:%s/%s' % (
                        user_db_conf['user'],
                        user_db_conf['password'],
                        user_db_conf['host'],
                        user_db_conf.get('port', 3306),
                        user_db_conf['database']
                      ), pool_recycle=3600)
db_session = scoped_session(sessionmaker(autocommit=False,
                                         autoflush=False,
                                         bind=engine))

# catch disconnects before they fuck things up
...
def toDict(self):
    ...
    fields = {}
    for field in [x for x in dir(self) if not x.startswith('_') and x not in {'metadata','query'} and x not in self.__serialize_exclude__]:
        ...
        if not hasattr(val, '__call__'):
            ...
            if isinstance(val, datetime.datetime):
                ...
                # AVOID CIRCULAR REFERENCES DEAR GOD
                val = getattr(self, field).dict()
...
Base.dict = toDict
Base.__serialize_exclude__ = set()
Base.__serialize_include__ = set()
...
##
# Json TYPE BECAUSE SQLALCHEMY IS RETARDED
##
...
def JsonType():
    return MutationObj.as_mutable(JsonEncodedObj)
